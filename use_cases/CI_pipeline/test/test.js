...
function createMarble(color, quantity = 1, cost = 1, callback) {
    cmd.get(
        'zowe console issue command "F ' + config.cicsRegion + ',' + config.cicsTran + ' CRE ' +
        color + " " + quantity + " " + cost + '" --cn ' + config.cicsConsole,
...
function updateMarble(color, quantity, callback) {
    cmd.get(
        'zowe console issue command "F ' + config.cicsRegion + ',' + config.cicsTran + ' UPD ' +
        color + " " + quantity + '" --cn ' + config.cicsConsole,
        function (err, data, stderr) {
            //log output
            var content = "Error:\n" + err + "\n" + "StdErr:\n" + stderr + "\n" + "Data:\n" + data;
...
describe('Marbles', function () {
    ...
    this.timeout(60000); // eslint-disable-line @typescript-eslint/no-magic-numbers
    ...
    describe('Inventory Manipulation', function () {
        ...
        it('should not create a marble of a color that already exists', function (done) {
            ...
            createMarble(COLOR, 2, 2, function (err, data, stderr) {
                if (err) {
                    ...
                    // Strip unwanted whitespace/newline
                    data = data.trim();
                    assert.equal(data, "+MARB002E Color (" + COLOR + ") already exists, UPDate or DELete it.",
                        "Unexpected marble creation or incorrect error message");
        ...
        it('should not be able to "redelete" the marble color from inventory', function (done) {
            ...
            deleteMarble(COLOR, function (err, data, stderr) {
                if (err) {
                    ...
                    // Strip unwanted whitespace/newline
                    data = data.trim();
                    assert.equal(data, "+MARB001E Color (" + COLOR + ") not found in inventory, CREate it.",
                        "Unexpected marble redeletion or incorrect error message");
        ...
        it('should not update marble inventory for a marble color that does not exist', function (done) {
            // Update marble
            updateMarble(COLOR, 3, function (err, data, stderr) { // eslint-disable-line @typescript-eslint/no-magic-numbers
                if (err) {
                    throw err;
                } else if (stderr) {
                    throw new Error("\nError: " + stderr);
                    ...
                    data = data.trim();
                    assert.equal(data, "+MARB001E Color (" + COLOR + ") not found in inventory, CREate it.",
                        "Unexpected marble update or incorrect error message");
